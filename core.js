  ...
  function initUnicornFadeIn() {
    ...
    const observer = new MutationObserver((mutations, obs) => {
      if (container.querySelector('canvas')) {
        setTimeout(() => gsap.to(container, { opacity: 1, duration: 0.8, ease: 'power2.out' }), 100);
        obs.disconnect();
      }
    });
    const existing = container.querySelector('canvas');
    if (existing) setTimeout(() => gsap.to(container, { opacity: 1, duration: 0.8, ease: 'power2.out' }), 100);
  ...
  function updateNavbarColors(targetNamespace) {
    ...
    if (!navbar) return;
    const ns = targetNamespace || document.querySelector('[data-barba-namespace]')?.getAttribute('data-barba-namespace');
  ...
  function initSplitLines() {
    ...
    cleanupSplitLines();
    function runSplit() {
      splitLinesInstance = new SplitType('.split-lines', { types: 'lines, words' });
      $('.line').each(function () {
        gsap.timeline({ scrollTrigger: { trigger: $(this), start: 'top center', end: 'bottom center', scrub: 1 } }).from($(this), { '--line-width': '0%', duration: 1 });
      });
    }
    runSplit();
    ...
    if (btn)
      btn.onclick = () => {
        if (window.lenis) {
          ...
          gsap.to(window, { scrollTo: { y: 0, autoKill: false }, duration: 2.5, ease: 'quart.inOut' });
        }
      };
  }
  // URL cleanup
      ...
      (e) => {
        ...
        if (link && !link.matches('.w-tab-link')) {
          ...
          if (target) {
            ...
            else gsap.to(window, { scrollTo: { y: target, autoKill: true }, duration: 1, ease: 'power2.inOut' });
            setTimeout(() => {
  ...
  function unreavealFlashElements() {
    return new Promise((resolve) => {
      const els = document.querySelectorAll('[data-flash-reveal][data-flash-initialized]');
      if (!els.length) {
        resolve();
        return;
      }
        ...
        gsap.timeline().to(chars, {
          ...
          stagger: {
            ...
            onStart: function () {
              ...
              for (let i = 0; i < flashes - 1; i++) ftl.to(char, { autoAlpha: 0, duration: 0.08, ease: 'none' }).to(char, { autoAlpha: 1, duration: 0.08, ease: 'none' });
            },
          },
        });
      });
  ...
  function disableLinks() {
    document.querySelectorAll('a').forEach((l) => {
      if (!l.hasAttribute('data-flash-hover-initialized')) l.style.pointerEvents = 'none';
    });
  }
  function enableLinks() {
    document.querySelectorAll('a').forEach((l) => (l.style.pointerEvents = 'auto'));
  }
  // Flash reveal
      ...
      tl.to(chars, {
        ...
        stagger: {
          ...
          onStart: function () {
            ...
            if (flashes === 1) return;
            const ftl = gsap.timeline();
            for (let i = 0; i < flashes - 1; i++) ftl.to(char, { autoAlpha: 1, duration: 0.08, ease: 'none' }).to(char, { autoAlpha: 0, duration: 0.08, ease: 'none' });
          },
        },
      });
      if (trigger === 'load') tl.play();
      else ScrollTrigger.create({ trigger: el, start: 'top 90%', onEnter: () => tl.play(), once: true });
    });
  }
  // Color reveal
  function initColorReveal() {
    if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') return;
    const CFG = { start: 'top 80%', end: 'top 20%', colors: { initial: '#d0d0d0', mid: '#EE7800', final: '#000000' } };
    ...
    function animChar(c) {
      ...
      gsap.killTweensOf(c);
      c._hasAnimated = true;
      gsap.timeline().to(c, { color: CFG.colors.mid, duration: 0.3, ease: 'power2.inOut' }).to(c, { color: CFG.colors.final, duration: 0.3, ease: 'power2.inOut' });
    }
        ...
        onUpdate: (self) => {
          ...
          if (down)
            ...
            for (let i = chars.length - 1; i >= 0; i--) {
              if (prog < (i + 1) / chars.length && chars[i]._hasAnimated) resetChar(chars[i]);
            }
          lastProg = prog;
        },
        onLeaveBack: () =>
  ...
  function closeMobileMenu() {
    ...
    if (top && mid && bot) {
      ...
      tl.to(mid, { opacity: 1, scale: 1, duration: 0.2, ease: 'power1.inOut' }, 0);
      tl.to(bot, { y: 8, rotateZ: 0, duration: 0.2, ease: 'power1.inOut' }, 0);
    }
    if (overlay) tl.to(overlay, { height: 0, duration: 0.2, ease: 'power1.inOut', onComplete: () => (overlay.style.display = 'none') }, 0);
  ...
  function initBrandsVideoHover() {
    document.querySelectorAll('.hover_video_wrapper .project-video').forEach((v) => {
  ...
  function initBestVentureButton() {
    ...
    btn.forEach((button) => {
      ...
      const st1 = ScrollTrigger.create({
        ...
        onLeaveBack: () => {
          gsap.to(button, { opacity: 0, duration: 0.4, ease: 'power4.out', onComplete: () => (button.style.display = 'none') });
        },
      });
  ...
  function initBrandsHeroReveal() {
    ...
    ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        imgs.forEach((img, i) => {
          let p = 0;
          if (prog >= i * seg && prog <= (i + 1) * seg) p = (prog - i * seg) / seg;
          else if (prog > (i + 1) * seg) p = 1;
          gsap.set(img, { maskImage: `linear-gradient(${90 + p * 40}deg, black ${50 - p * 50}%, transparent ${50 - p * 50}%, transparent ${50 + p * 50}%, black ${50 + p * 50}%)` });
        });
      },
    });
  }
  ...
  function initCountUpAnimations() {
    ...
    nums.forEach((n) => {
      ...
      new countUp.CountUp('number' + n[0], target, { enableScrollSpy: true, ...n[2] });
    });
  }
  // Brands button
  function initBrandsButtonAnimation() {
    document.querySelectorAll('.project-button').forEach((btn) => {
      ...
      btn.addEventListener('mouseenter', () => {
        ...
        if (left && right) tl.to(left, { scaleX: 1, duration: 2, ease: 'quint.out', delay: 0.2 }, 0).to(right, { scaleX: 0, duration: 2, ease: 'quint.out' }, 0);
        if (caretAp && caretDis) tl.to(caretAp, { x: '0%', duration: 1.75, ease: 'quint.inOut' }, 0).to(caretDis, { x: '100%', duration: 1.75, ease: 'quint.inOut' }, 0);
      });
    });
    document.querySelectorAll('.recruit--grid-block, .exect--card-wrapper').forEach((block) => {
  ...
  function initCardReveal() {
    document.querySelectorAll('[data-card-reveal]').forEach((card) => {
      ...
      gsap.set(card, { rotateZ: 5, clipPath: 'polygon(30% 30%, 70% 30%, 70% 70%, 30% 70%)' });
      ScrollTrigger.create({ trigger: card, start: 'top 100%', once: true, onEnter: () => gsap.to(card, { rotateZ: 0, clipPath: 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)', duration: 2, ease: 'power4.out', delay }) });
    });
  }
  // Mask wipe
  function initMaskWipeReveal() {
    document.querySelectorAll('[data-mask-wipe]').forEach((el) => {
      ...
      ScrollTrigger.create({ trigger: el, start: 'top 85%', once: true, onEnter: () => gsap.to(el, { clipPath: 'inset(0% 0% 0% 0%)', duration: 1.4, delay: 0.3, ease: CustomEase.create('smoothReveal', '0.25, 1, 0.35, 1') }) });
    });
  }
  ...
  function initRecruitSVG() {
    ...
    recruitSVGTl.to(trails, { opacity: 0, duration: 0.15, ease: 'power2.out' }, '+=0');
    recruitSVGTl.to([m, h], { drawSVG: '0% 100%', duration: 0.3, ease: 'power2.out' }, '<');
    recruitSVGTrigger = ScrollTrigger.create({ trigger: '.recruit--logo-svg', start: 'top 50%', once: true, onEnter: () => recruitSVGTl.play() });
  }
  // Pie chart
  ...
  function initPieChart() {
    ...
    cleanupPieChart();
    if (typeof ChartDeferred !== 'undefined' && !Chart.registry.plugins.get('deferred')) Chart.register(ChartDeferred);
    const lang = document.documentElement.lang;
    const labels = lang.startsWith('en') ? ['20s', '30s', '40s', '10s'] : ['20代', '30代', '40代', '10代'];
    ...
    pieChartInstance = new Chart(canvas.getContext('2d'), {
      type: 'doughnut',
      data: { labels, datasets: [{ data, backgroundColor: colors, borderWidth: 0, hoverBackgroundColor: colors, hoverOffset: 0 }] },
      options: {
        cutout: '50%',
        aspectRatio: 1,
        events: [],
        animation: { animateRotate: true, animateScale: true, onComplete: () => gsap.to('.chart-wrapper .chart-label', { opacity: 1, duration: 0.6, ease: 'power2.out', stagger: 0.1 }) },
        plugins: { deferred: { xOffset: 150, yOffset: '10%', delay: 200 }, legend: { display: false }, tooltip: { enabled: false } },
      },
      plugins: [
        {
          id: 'outsideLabels',
  ...
  function cleanupLottieInstances() {
    ...
    lottieInstances = [];
    document.querySelectorAll('[data-lottie-initialized]').forEach((el) => el.removeAttribute('data-lottie-initialized'));
  }
  ...
  function initRecruitSwiper() {
    ...
    document.querySelectorAll('.swiper2').forEach((el) => {
      const instance = new Swiper(el, { slidesPerView: 1, loop: true, speed: SPEED, allowTouchMove: false, effect: 'fade', fadeEffect: { crossFade: true } });
    ...
    const swiper = new Swiper(swiperEl, { slidesPerView: 3, loop: true, speed: SPEED, spaceBetween: 10, allowTouchMove: false });
    swiper.on('slideChange', () => {
      swiper2Instances.forEach((instance) => instance.slideToLoop(swiper.realIndex, SPEED));
    });
    ...
    swiperEl.addEventListener('click', (e) => {
      ...
      if (allSlides.indexOf(clickedSlide) - allSlides.indexOf(activeSlide) < 1) return;
      isAnimating = true;
      swiperEl.classList.add('is-animating');
      swiper.slideNext(SPEED);
      setTimeout(() => {
  ...
  function animateHeroExit() {
    return new Promise((resolve) => {
      ...
      if (cap3) tl.to(cap3, { opacity: 0, y: '1em', duration: 0.3, ease: 'power2.in' }, 0);
      if (cap2) tl.to(cap2, { opacity: 0, y: '1em', duration: 0.3, ease: 'power2.in' }, 0.05);
      if (cap1) tl.to(cap1, { opacity: 0, y: '1em', duration: 0.3, ease: 'power2.in' }, 0.1);
      if (heroDelay) {
        tl.to(heroDelay, { opacity: 0, duration: 0.3, ease: 'power2.in' }, 0.15);
        if (df) tl.to(df, { rotateX: 0, y: '0%', duration: 0.3, ease: 'power2.in' }, 0.15);
        if (db) tl.to(db, { rotateX: -90, y: '100%', duration: 0.3, ease: 'power2.in' }, 0.15);
      }
      if (hero) {
        tl.to(hero, { opacity: 0, duration: 0.3, ease: 'power2.in' }, 0.25);
        if (hf) tl.to(hf, { rotateX: 0, y: '0%', duration: 0.3, ease: 'power2.in' }, 0.25);
        if (hb) tl.to(hb, { rotateX: -90, y: '100%', duration: 0.3, ease: 'power2.in' }, 0.25);
      }
    });
  }
  // Hide hero elements
  function hideHeroElements(container) {
    ['hero-heading-home', 'heading-rotation-delay', 'hero-caption-first', 'hero-caption-second', 'hero-caption-third'].forEach((cls) => {
  ...
  function resetWebflow(data) {
    ...
    document.querySelectorAll('[data-color-reveal]').forEach((el) => {
      if (el.closest('[data-barba="container"]') && el._splitInstance) el._splitInstance.revert();
    ...
    document.querySelectorAll('[data-card-initialized]').forEach((el) => {
      ...
      gsap.set(el, { rotateZ: 5, clipPath: 'polygon(30% 30%, 70% 30%, 70% 70%, 30% 70%)' });
    });
    ...
    setTimeout(() => {
      ...
      const ns2 = document.querySelector('[data-barba-namespace]')?.getAttribute('data-barba-namespace');
      if (ns2 === 'brands' && typeof initBrandsHeroReveal === 'function') initBrandsHeroReveal();
  ...
  barba.hooks.enter((data) => {
    gsap.set(data.next.container, { position: 'fixed', top: 0, left: 0, width: '100%', visibility: 'hidden' });
  ...
  barba.init({
    ...
    timeout: 10000,
    prevent: ({ el }) => {
      return el.hasAttribute('fs-cmsload-element') || el.closest('[fs-cmsload-element]') !== null || el.hasAttribute('fs-list-element') || el.closest('[fs-list-element]') !== null || el.classList.contains('w-pagination-next') || el.classList.contains('w-pagination-previous') || el.closest('.w-pagination-wrapper') !== null;
    },
    transitions: [
      {
        ...
        to: { namespace: ['home'] },
        async leave(data) {
          if (typeof lockFlashHoverUnderlines === 'function') lockFlashHoverUnderlines();
          if (typeof lockMobileMenu === 'function') lockMobileMenu();
          disableLinks();
          hideHeroElements(data.next.container);
          await unreavealFlashElements();
          await gsap.to(data.current.container, { opacity: 0, duration: 0.5, ease: 'power2.inOut' });
        },
        async enter(data) {
          gsap.set(data.next.container, { visibility: 'visible', opacity: 0 });
          updateNavbarColors('home');
          await gsap.to(data.next.container, { opacity: 1, duration: 0.5, ease: 'power2.inOut' });
          enableLinks();
          if (typeof unlockFlashHoverUnderlines === 'function') unlockFlashHoverUnderlines();
      ...
      },
      {
        ...
        async leave(data) {
          ...
          if (typeof lockMobileMenu === 'function') lockMobileMenu();
          disableLinks();
          if (data.next.namespace === 'home') hideHeroElements(data.next.container);
          updateNavbarColors(data.next.namespace);
          await unreavealFlashElements();
          await gsap.to(data.current.container, { opacity: 0, duration: 0.5, ease: 'power2.inOut' });
        },
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    setTimeout(() => {
      ...
      initColorReveal();
      if (typeof initMaskWipeReveal === 'function') initMaskWipeReveal();
      if ((ns === 'brands' || ns === 'home' || ns === 'recruit' || ns === 'interview-cms') && typeof initBrandsButtonAnimation === 'function') initBrandsButtonAnimation();
      ...
      if (ns === 'home' && typeof initBestVentureButton === 'function') initBestVentureButton();
      if (ns === 'recruit') {
        if (typeof initRecruitSVG === 'function') initRecruitSVG();
        if (typeof initRecruitSwiper === 'function') initRecruitSwiper();
      }
